...
def adminlogin():
    ...
    if request.method == 'POST':
        if request.form['uname'] == 'admin' or request.form['password'] == 'admin':
            ...
            conn = mysql.connector.connect(user='root', password='', host='localhost', database='2socialnetpyemo',charset='utf8')
            # cursor = conn.cursor()
            cur = conn.cursor()
    ...
    if request.method == 'POST':
        ...
        if data is None:
            ...
            if data[10] == 'Blocked':
                ...
                conn = mysql.connector.connect(user='root', password='', host='localhost', database='2socialnetpyemo',charset='utf8',collation='utf8_general_ci')
                # cursor = conn.cursor()
                cur = conn.cursor()
...
def post1():
    if request.method == 'POST':
        ...
        mycursor.execute("insert into sharetb values('" + str(
            fnew) + "','" + uname + "','" + place + "','" + savename + "','" + uname + "','0')")
        conn.commit()
        conn.close()
        ...
        for x1 in data:
            ...
            mycursor = conn.cursor()
            mycursor.execute(
                "insert into sharetb values('" + str(
                    fnew) + "','" + uname + "','" + place + "','" + savename + "','" + fname + "','" + str(
                    out) + "')")
            conn.commit()
            conn.close()
            ...
            conn = mysql.connector.connect(user='root', password='', host='localhost', database='2socialnetpyemo')
            cursor = conn.cursor()
            cursor.execute("SELECT sum(Ccount) as count  FROM sharetb WHERE UserName  ='" + uname + "' ")
            ...
            if int(ccount) == 2:
                flash('You Have one More Time Post Unwanted Image')
                return 'You Have one More Time Post Unwanted Image..!'
                ...
                mycursor = mydb.cursor()
                mycursor.execute(
                    "update register set status='Blocked' where uname='" + str(uname) + "'")
                mydb.commit()
                mydb.close()
                return homepage()
